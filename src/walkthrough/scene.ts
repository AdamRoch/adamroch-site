...
import { setupRendererDebug, startLoop, watchQuality } from '../lab-quality';
...
// no canvas antialias — the frame is drawn into an MSAA HDR target and finished from there
...
// ACES over AgX: AgX flattens the dusk gradient and greys the moon (compared in screenshots)
...
// linear HDR → half-res bloom → tone map → grain, dither, vignette
...
sunGlow.material.color.setScalar(5); // an HDR disc: the core crosses the bloom threshold, the ring stays warm (1.3 read as a pale dot, 9 as a haze)
scene.add(sunGlow);
...
let waterNormal!: THREE.DataTexture; // baked in buildSea(); the loop scrolls it

function buildSea(): void {
...
const COLOSSUS_DIR = '/models/colossus/';
const COLOSSUS_URL = `${COLOSSUS_DIR}marble_bust_01_2k.gltf`;
const COLOSSUS_HEIGHT = 10.4; // metres, crown to plinth along its length

// every byte the bust needs, with the sizes on disk as a fallback when a
// server declines to send content-length — the progress rule is fed from these
const COLOSSUS_FILES: [file: string, bytes: number][] = [
  ['marble_bust_01_2k.gltf', 2793],
  ['marble_bust_01.bin', 416608],
  ['textures/marble_bust_01_diff_2k.jpg', 524864],
  ['textures/marble_bust_01_nor_gl_2k.jpg', 426860],
  ['textures/marble_bust_01_rough_2k.jpg', 652231],
];

// streams the model's files with byte-level progress, then hands the loader
// object urls for them through the manager, so it never fetches twice
async function prefetchColossus(
  manager: THREE.LoadingManager,
  onProgress: (fraction: number) => void
): Promise<() => void> {
  const totals = new Map(COLOSSUS_FILES);
  ...
  const tick = (): void => {
    let got = 0;
    let all = 0;
    for (const [file, bytes] of totals) {
      all += bytes;
      got += Math.min(bytes, loaded.get(file) ?? 0);
    }
    onProgress(all > 0 ? got / all : 1);
  };

  await Promise.all(
    COLOSSUS_FILES.map(async ([file]) => {
      const res = await fetch(COLOSSUS_DIR + file);
      if (!res.ok || !res.body) throw new Error(`${file}: ${res.status}`);
      const length = Number(res.headers.get('content-length'));
      if (length > 0) totals.set(file, length);
      ...
      blobs.set(absolute(COLOSSUS_DIR + file), URL.createObjectURL(new Blob(chunks, { type })));
    })
  );

  manager.setURLModifier((url) => blobs.get(absolute(url)) ?? url);
  return () => {
    for (const url of blobs.values()) URL.revokeObjectURL(url);
  };
}

function loadColossus(manager: THREE.LoadingManager): Promise<void> {
...
function buildMoonFace(): THREE.Group {
  ...
  for (const px of EYE_PX) {
    ...
    // both burn past white in linear space, so the bloom takes them
    const rim = new THREE.Mesh(
      new THREE.SphereGeometry(0.24, 14, 10),
      new THREE.MeshBasicMaterial({ color: new THREE.Color(0xe25822).multiplyScalar(1.0), fog: false })
    );
    rim.scale.set(1, 1, 0.4);
    const core = new THREE.Mesh(
      new THREE.SphereGeometry(0.17, 14, 10),
      new THREE.MeshBasicMaterial({ color: new THREE.Color(0xffb347).multiplyScalar(2.4), fog: false })
    );
    ...
    const glow = new THREE.Sprite(
      new THREE.SpriteMaterial({
        map: makeSunGlowTexture(),
        color: new THREE.Color(0xff8c30).multiplyScalar(4.0),
        blending: THREE.AdditiveBlending,
        depthWrite: false,
        transparent: true,
        opacity: 0.55,
...
// the aim and roll groups exist from the start so the egg can pose them before
// the face is painted; the face arrives in its own build stage
...
// the sign's group is seated and posed by the rite and the test hook; its wood
// and lettering are painted in a build stage
const omenSign = new THREE.Group();
omenSign.visible = false;
scene.add(omenSign);

function buildOmenSign(): void {
  const group = omenSign;
...
/* ————— first-person controls: pointer lock, wasd, arrows, a finger, sprint, damped velocity ————— */

const WALK_SPEED = 5.0;
const SPRINT_SPEED = 9.0;
const MOUSE_SENS = 0.0022;
const TOUCH_SENS = 0.0042; // radians per css pixel — a thumb's sweep turns you a quarter round
const ARROW_YAW = 1.5; // radians per second
const ARROW_PITCH = 1.0;
const DOLLY_SPEED = 0.85; // metres per second — the phone walk is an approach, not a sprint
const DOLLY_STOP = 6.5; // metres short of the head's centre
...
let yaw = 0;
let pitch = 0;
let locked = false; // the pointer is ours
let live = false; // the walker is in the scene: locked, or entered without a lock (touch, keyboard, a refused lock)
let dollying = false; // touch entry: the walk carries you toward the head until you take over
let sceneReady = false; // the head is seated and the first frame has landed
...
if (reduced) {
  if (helpLine) helpLine.textContent = 'Reduced motion is on, so the scene holds as a single still frame.';
  if (enterBtn) enterBtn.textContent = 'View the scene';
} else if (!finePointer) {
  if (helpLine) helpLine.textContent = 'Drag to look around. The walk carries you down the beach to the head.';
}

function setEnterReady(ready: boolean): void {
  if (!enterBtn) return;
  enterBtn.setAttribute('aria-disabled', String(!ready));
}

// entry without a pointer lock: a finger, a keyboard, or a lock the browser refused
function enterFree(): void {
  if (overlay) overlay.hidden = true;
  live = true;
  dollying = !finePointer;
  applySound(); // entry is the user gesture audio needs
}

function leaveFree(): void {
  live = false;
  dollying = false;
  clearKeys();
  if (overlay) overlay.hidden = false;
  if (enterBtn) {
    enterBtn.textContent = 'Step back in';
    enterBtn.focus();
  }
}
...
function requestLock(): void {
  ...
  if (req instanceof Promise) req.catch(() => enterFree()); // a refused lock still lets you in, on the keys
}

overlay?.addEventListener('click', (e) => {
  if ((e.target as HTMLElement | null)?.closest('a')) return; // let the way home stay a link
  if (!overlay || !sceneReady) return;
  if (reduced) {
    overlay.hidden = true; // a still frame — nothing to enter, just step aside
    return;
  }
  if (!finePointer) {
    enterFree();
...
// after a refused or lost lock, a click on the scene asks for it again
canvas.addEventListener('click', () => {
  if (live && !locked && finePointer && !reduced) requestLock();
});
...
document.addEventListener('pointerlockchange', () => {
  ...
  if (locked) {
    ...
    // the lock let go (Escape, tab switch): back to the card
    velocity.set(0, 0, 0);
    leaveFree();
  }
});
...
// one finger grabs the world: drag right and the beach slides right under your eye
let dragId: number | null = null;
let dragX = 0;
let dragY = 0;

canvas.addEventListener('pointerdown', (e) => {
  if (!live || locked || e.pointerType === 'mouse') return;
  dragId = e.pointerId;
  dragX = e.clientX;
  dragY = e.clientY;
...
canvas.addEventListener('pointermove', (e) => {
  ...
  yaw += (e.clientX - dragX) * TOUCH_SENS;
  pitch += (e.clientY - dragY) * TOUCH_SENS;
  dragX = e.clientX;
  dragY = e.clientY;
  clampPitch();
});
...
function setKey(code: string, down: boolean): boolean {
  switch (code) {
    ...
    case 'ShiftLeft':
    case 'ShiftRight': keys.sprint = down; break;
    case 'ArrowLeft': keys.yawLeft = down; break;
    case 'ArrowRight': keys.yawRight = down; break;
    case 'ArrowUp': keys.pitchUp = down; break;
    case 'ArrowDown': keys.pitchDown = down; break;
...
window.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && live && !locked) {
    if (notePanel && !notePanel.hidden) return; // the note owns this Escape
    leaveFree();
    return;
  }
  if (setKey(e.code, true) && live) {
    dollying = false; // the walker takes over from the dolly at the first key
    e.preventDefault();
  }
});
...
/* ————— the design note is wired by the shared chrome; the page only checks whether it is open ————— */
...
function moveWalker(dt: number): void {
  // the arrows turn the head at a steady rate — a keyboard-only walker can still look around
  if (keys.yawLeft) yaw += ARROW_YAW * dt;
  if (keys.yawRight) yaw -= ARROW_YAW * dt;
  if (keys.pitchUp) pitch += ARROW_PITCH * dt;
  if (keys.pitchDown) pitch -= ARROW_PITCH * dt;
  clampPitch();
  ...
  if (live) {
    ...
    tx = (forward.x * iz + strafe.x * ix) * speed;
    tz = (forward.z * iz + strafe.z * ix) * speed;
    ...
    if (dollying && len === 0) {
      // the phone walk: a slow approach that eases off short of the head
      const dx = HEAD_X - camera.position.x;
      const dz = HEAD_Z - camera.position.z;
      const d = Math.hypot(dx, dz);
      const go = THREE.MathUtils.smoothstep(d, DOLLY_STOP, DOLLY_STOP + 3) * DOLLY_SPEED;
      if (go > 0.01) {
        tx = (dx / d) * go;
        tz = (dz / d) * go;
      } else {
  ...
  velocity.x += (tx - velocity.x) * blend;
  velocity.z += (tz - velocity.z) * blend;
...
/* ————— adaptive quality: resolution first, then the bloom, then the shadow's reach ————— */

let qualityTier = 0;
let setStatus: (text: string) => void = () => {};

function setPixelRatioCap(cap: number): void {
  const ratio = Math.min(window.devicePixelRatio, cap);
  renderer.setPixelRatio(ratio);
  post.setSize(window.innerWidth, window.innerHeight, ratio);
}
...
function stepQualityDown(): void {
  ...
  if (qualityTier === 1) {
    ...
    sun.shadow.map = null; // the next frame allocates the smaller map
  }
}
...
function frame(rawDt: number, t: number): void {
  ...
  if (shakeAmp > 0.0002) {
    camera.rotation.x += Math.sin(t * 39.7) * shakeAmp;
    camera.rotation.y += Math.sin(t * 31.3 + 1.7) * shakeAmp * 0.8;
    camera.rotation.z += Math.sin(t * 43.1 + 3.9) * shakeAmp * 0.5;
  }
  ...
  if (now - fpsLast > 500) {
    fpsLast = now;
    setStatus(`FPS ${String(Math.round(fpsEma)).padStart(2, '0')} · QUALITY ${qualityTier}`);
  }
...
window.addEventListener('resize', () => {
  ...
  if (reduced && sceneReady) post.render(0);
});

/* ————— boot: real work feeds the curtain, and the first frame lets it go ————— */

const nextFrame = (): Promise<void> => new Promise((r) => requestAnimationFrame(() => r()));

// every texture the scene owns, so the uploads can be spread over frames
function collectTextures(): THREE.Texture[] {
  ...
  scene.traverse((o) => {
    ...
    for (const m of mats) {
      ...
      for (const key of ['map', 'normalMap', 'roughnessMap', 'metalnessMap', 'aoMap', 'emissiveMap']) {
        const tex = rec[key];
        if (tex instanceof THREE.Texture) found.add(tex);
      }
    }
  });
  return [...found];
...
export async function boot(pre: Preloader, chrome: LabChrome): Promise<void> {
  setStatus = (text) => chrome.setStatus(text);

  // the bytes stream in while the beach is built; both feed the one rule
  let bytes = 0;
  let stages = 0;
  const STAGES = 4;
  const report = (label: string): void => pre.set(0.02 + bytes * 0.5 + (stages / STAGES) * 0.3, label);

  const manager = new THREE.LoadingManager();
  const fetching = prefetchColossus(manager, (f) => {
    bytes = f;
    report('fetching the colossus');
  }).catch(() => {
    bytes = 1; // the loader fetches on its own if the stream failed
    return () => {};
  });

  report('raising the dunes');
  buildGround();
  stages++;
  await nextFrame();

  report('settling the sea');
  buildSea();
  stages++;
  await nextFrame();

  report('painting the moon');
  buildMoon();
  stages++;
  await nextFrame();

  report('lettering the sign');
  buildOmenSign();
  stages++;
  await nextFrame();

  const release = await fetching;
  report('seating the head');
  await loadColossus(manager);
  release();

  // gpu warm-up: one texture upload per frame, then the shaders in parallel
  const textures = collectTextures();
  for (let i = 0; i < textures.length; i++) {
    renderer.initTexture(textures[i]);
    pre.set(0.82 + (i / textures.length) * 0.1, 'uploading textures');
    await nextFrame();
  }
  pre.set(0.92, 'compiling shaders');
  await post.compile(scene, camera);
  pre.set(0.98, 'first light');

  camera.rotation.set(pitch, yaw, 0);
  post.render(0);
  sceneReady = true;
  setEnterReady(true);
  void pre.done(); // the curtain fades over a live frame

  if (reduced) {
    // one considered frame, no perpetual motion
    setStatus('STILL FRAME');
    return;
  }

  startLoop(frame);
  // a test hook owns the frame it holds — tiering would make its screenshots drift
  if (!eggTest && !signTest) watchQuality(stepQualityDown, { maxDrops: 3 });
}
